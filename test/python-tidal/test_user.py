...
def test_get_user(session):
    ...
    asserted_fav_url = 'users/' + str(user_id) + '/favorites'
    assert user.favorites.base_url == asserted_fav_url
    assert user.first_name is None # == "Five Dragons"
    assert user.last_name is None # == "Music"
    assert not user.picture_id

def test_get_user_favorites(session):
    user = session.get_user(user_id)
    assert isinstance(user.favorites, tidalapi.Favorites)
    assert user.favorites.tracks()
    assert user.favorites.albums()
    assert user.favorites.tracks()
    assert user.favorites.artists()
    assert user.favorites.videos()
